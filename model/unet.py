...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.conv = ConvBlock(in_channels, out_channels) # in_channels from concat: (in_channels // 2) + (in_channels // 2)
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=False):
        ...
        self.sigmoid = nn.Sigmoid() # Added for binary segmentation output in [0,1]
    ...
    def forward(self, x):
        ...
        x = self.up4(x, x1)
        logits = self.outc(x)
        if self.n_classes == 1: # Apply sigmoid for binary segmentation
            return self.sigmoid(logits)
        else: # For multi-class, raw logits are usually preferred (e.g. with CrossEntropyLoss)
            return logits
